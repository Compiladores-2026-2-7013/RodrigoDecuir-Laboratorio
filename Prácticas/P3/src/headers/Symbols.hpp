 ...
 * declaracion, tipo, lista_var, lista_var', sentencia, expresion,
 * expresion', termino, termino', factor }
 *
 * T = { identificador, int, float, if, (, ), while, *, -, +, /, numero,
 * else, ;, ',', = }
 *
 * P' = {
...
enum Token { primero, t_eof, t_id, t_int, t_float, t_if, t_par_izq,
              t_par_der, t_while, t_mult, t_menos, t_mas, t_div, t_numero,
              t_else, t_punto_y_coma, t_coma, t_asig, t_error };

/**
 * 3. Determinar la enumeración para el conjunto N
 */
enum NoTerm { strEmpty, nt_programa, nt_declaraciones, nt_declaraciones_prima,
              nt_sentencias, nt_sentencias_prima, nt_declaracion, nt_tipo, nt_lista_var,
              nt_lista_var_prima, nt_sentencia, nt_expresion, nt_expresion_prima,
